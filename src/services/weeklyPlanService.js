// Weekly Plan Service - Manages AI-generated health plans with weekly resets
// Falls back to default plans if AI generation fails

import { generateWeeklyPlan } from './gemini';
...
class WeeklyPlanService {
    ...
    // Generate a new plan using AI (always falls back to defaults on error)
    async generateNewPlan() {
        try {
            ...
            return { success: true, plan: this.currentPlan };
        } catch (error) {
            console.warn('AI plan generation failed, using default plan:', error.message);
            // Always fall back to defaults - never throw
            this.currentPlan = DEFAULT_PLAN;
            this.savePlan();
            this.notifyListeners();
            return { success: false, plan: this.currentPlan, error: error.message };
        }
    }

    // Check if we need a new plan and generate if necessary
    async checkAndResetWeek() {
        if (isNewWeek()) {
            console.log('New week detected! Attempting to generate new plan...');
            try {
                await this.generateNewPlan();
            } catch (error) {
                console.warn('Failed to generate new plan on week reset, using defaults');
            }
            // Reset progress for all modules
    ...
    resetWeeklyProgress() {
        ...
        localStorage.removeItem('weeklyWorkoutCompletion');
        // Daily habits keep their streak but reset daily progress
        const habitsData = localStorage.getItem('dailyHabitsData');
        if (habitsData) {
            const data = JSON.parse(habitsData);
